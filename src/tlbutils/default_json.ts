...
import { JSONObject, JSONValue } from './json_to_base64';
...
export async function getDefaulHumanJsonUnsafe(tlbCode: TLBCode, tlbType: TLBType) {
    ...
    return jsonGen.addTlbType(tlbType);
}
...
export class DefaultJsonGenerator {
    ...
    addTlbType(tlbType: TLBType): JSONObject | undefined {
        let ctx: JsonContext = {
            constructorsReached: new Set<string>(),
            constructorsCalculated: new Map<string, number>(),
        };

        return this.getTLBTypeNameResult(tlbType.name, ctx, []);
    }

    getTLBTypeNameResult(tlbTypeName: string, ctx: JsonContext, parameters: TLBMathExpr[]): JSONObject | undefined {
    ...
    getTLBTypeResult(tlbType: TLBType, ctx: JsonContext, parameters: TLBMathExpr[]): JSONObject | undefined {
    ...
    handleField(field: TLBField, x: JSONObject, ctx: JsonContext, y: Map<string, TLBMathExpr>): boolean {
    ...
    handleType(
        ...
        let res: JSONValue | undefined = undefined;
