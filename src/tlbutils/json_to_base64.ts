...
export async function humanJsonToBase64(typeName: string, tlbCode: TLBCode, json: JSONObject, method: any) {
    let s = new X();
    ...
    method(s.jsonToType(typeName, tlbCode, json))(builder);
    return builder.asCell().toBoc().toString('base64');
}

class X {
    Address: never | undefined;
    Cell: never | undefined;
    Dictionary: never | undefined;
    beginCell: () => Builder;
    ...
    jsonToType(kindName: string, tlbCode: TLBCode, json: JSONObject) {
    ...
    getTLBTypeNameResult(kindName: string, tlbCode: TLBCode, json: JSONObject, parameters: TLBFieldType[]) {
        let constructor = constructorsIndex.get(kindName);
        if (constructor) {
    ...
    getTLBConstructorResult(
        ...
        constructor.fields.forEach((field: TLBField) => {
            let json_to_pass = json[field.name] ? json[field.name] : json;
            Object.assign(result, this.handleField(field, tlbCode, json_to_pass, y));
        });
        return result;
    }

    handleField(field: TLBField, tlbCode: TLBCode, json: JSONObject, y: Map<string, TLBFieldType>) {
